...
public class Testing{
    ...
    public static String[] produceAnswer(String input){
    	...
    	if(operator.equals("+")||operator.equals("-")){
    		...
    		operatorCalc = simplify(operatorCalc);
   ...
   public static int[] simplify(int[] newFraction){
	int[] returnAnswer;
   	int whole = newFraction[0];
   	int numerator = newFraction[1];
   	int denominator = newFraction[2];
   	if(newFraction.length == 1 || numerator == 0){ //if the length equals one then it would just be the whole number
   		returnAnswer = new int[1];
   		returnAnswer[0] = whole;
   		return returnAnswer;
   	}
   	if(numerator < 0 && whole != 0){ // if hole number exists and numerator is negative 
   	...
   	for(int i = gcfNum + 1; i < denominator; i++){
   ...
   public static int gcf(int numOne, int numTwo){//calculates for the greatest common factor
